...
the sanitation index.  The 2015 catch-only projections used the same version of SS2 as the 2005 assessment model.  
...
more weight given to the model without the sanitation district survey.  All 
...
prior to when length compoisition are available.
...
were considered.  However, female natural mortality was estimed at 0.38, much too high to 
be considered a reasonable value.  The age data needed to estimate natural mortality (especially 
for older fish) is not yet available.  Male natural mortality was estimatable as an offset from 
...
was fixed. Steepness was estiamted at approximately 0.8.  No data exist to inform this parameter 
for California scorpfionsh, and the decision was made to fix steepness at the mean of the prior 
developed from a meta-anaylsis of West Coast groundfish.
...
was repeated 100 times and the minimum was reached in 45% of the runs  (Table \ref{tab:jitter}).  
...
assessment, as well as a number of sentivities to modelling choices made in developing the 
...
length and age composition data.  A model run with defauly weighting increased the total 
likelihood by 2,692 and resulted in a final depletion of 0.771.  The base model used the 
...
The sanitation district index was the axis of uncertainty in teh 2005 assessment.  The current assessment 
has a number of new data sources, including new indices, length data, and conditional age-at-length data 
available.  Removing the sanitation index and lenght composition data from the current base model 
